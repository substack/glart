...
function smoke (regl) {
  ...
  return regl({
    frag: `
      ...
      varying vec3 vpos;
      void main () {
        float c = snoise(curlNoise(vpos-vec3(0,time*0.25,0))) * 0.2
          * (1.0 - vpos.y/4.0)
        ;
        gl_FragColor = vec4(c,c,c,c*0.5);
      }
    `,
    vert: `
      precision mediump float;
      ...
      ${snoise}
      void main () {
        vec3 offset = vec3(0,-2.4,0);
        float c = (snoise(position*2.0-vec3(0,time*0.25,0))*2.0-1.0)*0.1+0.5;
